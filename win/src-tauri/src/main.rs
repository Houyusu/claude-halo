            ...
            tauri::async_runtime::spawn(async move {
                ...
                loop {
                    ...
                    if matches!(new_state, HaloState::Completed) {
                        ...
                        // When the terminal HWND is stale (e.g. after compaction
                        // restart), verify via process association instead of
                        ...
                        if hold_completed && effectively_focused {
                            ...
                            if !focus_valid {
                                // focus_hwnd was stale, re-capture the verified terminal
                                focus_hwnd = fg;
                                focus_captured = true;
                            }
                            hold_completed = false;
